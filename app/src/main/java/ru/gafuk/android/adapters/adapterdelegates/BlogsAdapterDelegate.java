...
public class BlogsAdapterDelegate extends AdapterDelegate<List<BlogItem>> {

    private final DateUtils dateUtils = DateUtils.getInstance();
    private final String itemView;

    public BlogsAdapterDelegate() {itemView = Preferences.Blog.getItemView();}

    @Override
    protected boolean isForViewType(@NonNull List<BlogItem> items, int position) {
...
//        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.cardview_compat_item, parent, false));
    ...
    @Override
    protected void onBindViewHolder(@NonNull List<BlogItem> items, int position, @NonNull RecyclerView.ViewHolder holder, @NonNull List<Object> payloads) {
